...
if [[ $GIT_KSU_STATE = 'true' ]]; then
	if [ ! -d $(pwd)/KernelSU ]; then
		if [[ $GIT_KSU_BRANCH = 'dev' ]]; then
			curl -LSs "https://raw.githubusercontent.com/rsuntk/KernelSU-Legacy/main/kernel/setup.sh" | bash -s main
		elif [[ $GIT_KSU_BRANCH = 'stable' ]]; then
			curl -LSs "https://raw.githubusercontent.com/rsuntk/KernelSU-Legacy/main/kernel/setup.sh" | bash -
		fi
	fi
